function __print_portalrom_functions_help() {
cat <<EOF
Additional PortalRom functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- portalromgerrit:   A Git wrapper that fetches/pushes patch from/to PortalRom Gerrit Review.
- portalromrebase:   Rebase a Gerrit change and push it again.
- portalromremote:   Add git remote for PortalRom Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for PortalRom Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the PortalRom model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch portalrom_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/portalrom-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.portalrom.device | grep -q "$PORTALROM_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $PORTALROM_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function portalromremote()
{
    ...
    git remote rm portalrom 2> /dev/null
    local REMOTE=$(git config --get remote.github.projectname)
    local PORTALROM="true"
    if [ -z "$REMOTE" ]
    then
    ...
    then
        ...
        local PFX="PortalRomOS/"
    else
        local PROJECT=$REMOTE
    fi

    local PORTALROM_USER=$(git config --get review.review.portalromos.org.username)
    if [ -z "$PORTALROM_USER" ]
    then
        git remote add portalrom ssh://review.portalromos.org:29418/$PFX$PROJECT
    else
        git remote add portalrom ssh://$PORTALROM_USER@review.portalromos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'portalrom' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/PortalRomOS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.portalrom.device | grep -q "$PORTALROM_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        portalromremote
        git push portalrom HEAD:refs/heads/'$1'
    fi
    '
}

function portalromgerrit() {
    ...
    local user=`git config --get review.review.portalromos.org.username`
...
EOF
            ...
            case $1 in
                ...
                changes|for)
                    if [ "$FUNCNAME" = "portalromgerrit" ]; then
...
function portalromrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "PortalRomOS Gerrit Rebase Usage: "
        echo "      portalromrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.portalromos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.portalrom.device | grep -q "$PORTALROM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/portalrom/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $PORTALROM_FIXUP_COMMON_OUT ]; then
