...
// Copyright (C) 2018,2021 The PortalRom Project
//
...
func init() {
	android.RegisterModuleType("portalrom_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = portalromExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = portalromExpandVariables(ctx, dep_file)
	...
	cmd := portalromExpandVariables(ctx, String(g.properties.Cmd))
